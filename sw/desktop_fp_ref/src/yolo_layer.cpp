...
layer make_yolo_layer(int batch, int w, int h, int n, int total, int *mask, int classes)
{
    ...
    l.biases = (fp_weight_type *)calloc(total*2, sizeof(fp_weight_type));
    if(mask) l.mask = mask;
    else{
    ...
    l.output = (fp_data_type *)calloc(batch*l.outputs, sizeof(fp_data_type));
    for(i = 0; i < total*2; ++i){
        l.biases[i] = .5;
    }
...
void delta_yolo_class(float *output, float *delta, int index, int class_idx, int classes, int stride, float *avg_cat)
{
    int n;
    if (delta[index]){
        delta[index + stride*class_idx] = 1 - output[index + stride*class_idx];
        if(avg_cat) *avg_cat += output[index + stride*class_idx];
        return;
    }
    for(n = 0; n < classes; ++n){
        delta[index + stride*n] = ((n == class_idx)?1 : 0) - output[index + stride*n];
        if(n == class_idx && avg_cat) *avg_cat += output[index + stride*n];
    }
}
...
{
    int b,n;
    memcpy(l.output, net.input, l.outputs*l.batch*sizeof(fp_data_type));
    for (b = 0; b < l.batch; ++b){
        for(n = 0; n < l.n; ++n){
    ...
    FILE *LayerOutput=fopen("yolo_output_fp_sdk.dat","w");
...
int yolo_num_detections(layer l, float thresh)
{
    ...
    for (i = 0; i < l.w*l.h; ++i){
        for(n = 0; n < l.n; ++n){
            ...
            if(l.output[obj_index] > (fp_data_type)thresh){
                ++count;
            }
        }
    }
...
{
    int i,j,n;
    fp_data_type *predictions = l.output;
