...
#pragma HLS INTERFACE axis register both port=inStream
	...
	line_buff_type line_buff_group_0[INPUT_CHANNEL/4];
	line_buff_type line_buff_group_1[INPUT_CHANNEL/4];
	line_buff_type line_buff_group_2[INPUT_CHANNEL/4];
	line_buff_type line_buff_group_3[INPUT_CHANNEL/4];

	window_type window_group_0[KERNEL_NUM/4];
#pragma HLS ARRAY_PARTITION variable=window_group_0 complete dim=1
	window_type window_group_1[KERNEL_NUM/4];
#pragma HLS ARRAY_PARTITION variable=window_group_1 complete dim=1
	window_type window_group_2[KERNEL_NUM/4];
#pragma HLS ARRAY_PARTITION variable=window_group_2 complete dim=1
	window_type window_group_3[KERNEL_NUM/4];
#pragma HLS ARRAY_PARTITION variable=window_group_3 complete dim=1


	fp_data_type val_output_0[KERNEL_NUM/4];
#pragma HLS ARRAY_PARTITION variable=val_output_0 complete dim=1
	fp_data_type val_output_1[KERNEL_NUM/4];
#pragma HLS ARRAY_PARTITION variable=val_output_1 complete dim=1
	fp_data_type val_output_2[KERNEL_NUM/4];
#pragma HLS ARRAY_PARTITION variable=val_output_2 complete dim=1
	fp_data_type val_output_3[KERNEL_NUM/4];
#pragma HLS ARRAY_PARTITION variable=val_output_3 complete dim=1
	...
	for(int row_stride=0;row_stride<STRIDE;row_stride++)
	{
		...
		{
			for(int input_ch_idx=0;input_ch_idx<INPUT_CHANNEL/4;input_ch_idx++)
			{
#pragma HLS PIPELINE
				...
				if((row_idx>=KERNEL_DIM-1)&&(col_idx>=KERNEL_DIM-1)&&(row_stride==STRIDE-1)&&(col_stride==STRIDE-1))
				{
					...
					window_group_2[input_ch_idx] = slide_window(conv_count,&line_buff_group_2[input_ch_idx]);
					window_group_3[input_ch_idx] = slide_window(conv_count,&line_buff_group_3[input_ch_idx]);
					...
					val_output_2[input_ch_idx] = window_max_pool(&window_group_2[input_ch_idx]);
					val_output_3[input_ch_idx] = window_max_pool(&window_group_3[input_ch_idx]);

					ap_uint<1> last;
					if((out_row==OUTPUT_HEIGHT+POOL_PAD-1)&&
					   (out_col==OUTPUT_WIDTH+POOL_PAD-1)&&
					   (input_ch_idx==INPUT_CHANNEL/4-1))
					{
						last = 1;
					}
					else
					...
					write_output(val_output_0[input_ch_idx],val_output_1[input_ch_idx],val_output_2[input_ch_idx],val_output_3[input_ch_idx],curr_input,outStream,last);
				}
