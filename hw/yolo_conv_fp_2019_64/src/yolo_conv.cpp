...
#pragma HLS STREAM variable=out_stream_group depth=2 dim=1
	...
	line_buff_type line_buff_group_0[INPUT_CHANNEL/4+1];
	line_buff_type line_buff_group_1[INPUT_CHANNEL/4+1];
	line_buff_type line_buff_group_2[INPUT_CHANNEL/4+1];
	line_buff_type line_buff_group_3[INPUT_CHANNEL/4+1];
...
#pragma HLS ARRAY_PARTITION variable=window_group_1 complete dim=1
	window_type window_group_2[KERNEL_NUM];
#pragma HLS ARRAY_PARTITION variable=window_group_2 complete dim=1
	window_type window_group_3[KERNEL_NUM];
...
#pragma HLS ARRAY_PARTITION variable=sub1_val_output complete dim=1
	fp_data_type sub2_val_output[KERNEL_NUM];
#pragma HLS ARRAY_PARTITION variable=sub2_val_output complete dim=1
	fp_data_type sub3_val_output[KERNEL_NUM];
...
//#pragma HLS ARRAY_PARTITION variable=local_mem_group cyclic factor=2 dim=2
...
#pragma HLS PIPELINE
	...
	for(int i=0;i<OUTPUT_CHANNEL/4;i++)//division 2 is not safe here!!!
	{
#pragma HLS PIPELINE
		curr_input = inStream.read();
				...
				if(row_idx != INPUT_HEIGHT)
				{
					...
					if((row_idx>KERNEL_DIM-2)&&(col_idx>KERNEL_DIM-2))
					{
						...
						for(int kernel_idx=0; kernel_idx<KERNEL_NUM; kernel_idx++)
						{
							...
							if(input_ch_idx == (INPUT_CHANNEL+3)/4-1)
							{
...
//				   ((row_idx==INPUT_HEIGHT)&&(col_idx==0)))
                ...
                if(((row_idx>KERNEL_DIM-1)&&(col_idx>=KERNEL_DIM-1))||((row_idx==KERNEL_DIM-1)&&(col_idx>KERNEL_DIM-1)))
				{
                	//printf("%d,%d\n",row_idx,col_idx);
    				ap_uint<1> last;
    				if((row_idx==INPUT_HEIGHT)&&(input_ch_idx==(INPUT_CHANNEL+3)/4-1))
    					last = 1;
    				else
    					last = 0;
	...
	for(int i=0; i<STREAM_TX_SIZE; i++)
	{
		...
		if(4*kernel_idx<KERNEL_NUM)
			if(!(out_stream_group[4*kernel_idx].empty()))
			{
				...
				if(4*kernel_idx == KERNEL_NUM-1||4*kernel_idx+1 == KERNEL_NUM-1||4*kernel_idx+2 == KERNEL_NUM-1||4*kernel_idx+3 == KERNEL_NUM-1)
