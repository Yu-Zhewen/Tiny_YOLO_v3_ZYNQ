...
{
#pragma HLS ARRAY_PARTITION variable=kernel_weight_fp block factor=16 dim=1
...
#pragma HLS ARRAY_PARTITION variable=activated_output complete dim=1
	...
	fp_weight_type kernel_weight_fp[OUTPUT_CHANNEL*INPUT_CHANNEL*KERNEL_DIM*KERNEL_DIM];
	fp_weight_type kernel_bias_fp[OUTPUT_CHANNEL];

	for(int i=0;i<OUTPUT_CHANNEL*INPUT_CHANNEL*KERNEL_DIM*KERNEL_DIM;i++)
	{
...
#pragma HLS PIPELINE II=1
				...
				if(row_idx != INPUT_HEIGHT)
				{
					...
					if((row_idx>=KERNEL_DIM-1)&&(col_idx>=KERNEL_DIM-1))
					{
						...
						for(int kernel_idx=0; kernel_idx<KERNEL_NUM; kernel_idx++)
						{
							...
							//core of conv, macc
							sub0_val_output[kernel_idx] += window_macc(&window_group_0[kernel_idx],&kernel_weight_fp[kernel_idx*KERNEL_AREA+2*input_ch_idx*KERNEL_DIM*KERNEL_DIM]);
							if(!(((INPUT_CHANNEL+1)/2!=(INPUT_CHANNEL)/2)&&(input_ch_idx == (INPUT_CHANNEL+1)/2-1)))
								sub1_val_output[kernel_idx] += window_macc(&window_group_1[kernel_idx],&kernel_weight_fp[kernel_idx*KERNEL_AREA+(2*input_ch_idx+1)*KERNEL_DIM*KERNEL_DIM]);

							//val_output[kernel_idx] = sub0_val_output[kernel_idx] + sub1_val_output[kernel_idx];

							//accumulate for number of input channels
							if(input_ch_idx == (INPUT_CHANNEL+1)/2-1)
							{

								//biased_output[kernel_idx] = val_output[kernel_idx] + kernel_bias_fp[kernel_idx];
				...
				{
					ap_uint<1> last;
					if((row_idx==INPUT_HEIGHT)&&(col_idx==0)&&(input_ch_idx==(INPUT_CHANNEL+1)/2-1))
						last = 1;
					else
						last = 0;
...
fp_data_type window_macc(window_type *window, fp_weight_type weight[WINDOW_AREA])
{
	...
	for(int win_row=0; win_row < KERNEL_DIM; win_row++)
	{
		for(int win_col=0; win_col < KERNEL_DIM; win_col++)
		{
			...
			fp_data_type val_out = val_in * weight[win_row*KERNEL_DIM+win_col];
			sum += val_out;
		}
	}
